...
## CorrelateOmics
```CorrelateOmics(ProteomicsDataFilePath = "LFQ_intensities.xlsx", UniProtIDColumnName = "Protein IDs", GeneNameColumnName = "Gene name", ProteomicsColumnsToCalculateMean, TranscriptomicsDataFilePath, TranscriptomicsColumnsToCalculateMean, dataset)```

```plotCorrelateOmics(DataFrame, Alpha = 0.1, HighlightGeneNameRegex, HighlightAlpha = 1, HighlightColor = "#C40233", HighlightSize = 2.5)```

```CorrelateOmics``` correlates proteomics data from ```ProteomicsDataFilePath``` and transcriptomics data from ```TranscriptomicsDataFilePath``` of each gene from the species referred in the BioMart ```dataset```. Only genes with a one-to-one mapping are included. The result is a data frame with 5 columns: "logTranscriptomicsMean", "logTranscriptomicsStdev", "logProteomicsMean", "logProteomicsStdev", and (NCBI) "GeneName". The row names of the data frame are the Ensembl IDs. ```plotCorrelateOmics``` can then plot the resulting data frame. If we want to highlight certain genes, we can specify them by their NCBI gene names using the ```HighlightGeneNameRegex```. The returned ```ggplot``` object can also be viewed interactively by ```ggplotly```.
