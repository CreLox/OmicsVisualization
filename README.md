...
Converts a single Ensembl ID to its corresponding NCBI Entrez accession(s)/ID(s)/description(s) (```Output = c("Accession", "ID", "Description")```) using the [rentrez](https://docs.ropensci.org/rentrez/) package. If the mapping exists, the output will be a string array; otherwise, the output will be ```""```. This works better than using ```biomaRt``` because the mapping is more complete. And unlike using ```org.X.eg.db```, this works for all species.
