...
## CorrelateOmics
```CorrelateOmics(ProteomicsDataFilePath = "LFQ_intensities.xlsx", UniProtIDColumnName = "Protein IDs", GeneNameColumnName = "Gene name", ProteomicsColumnsToCalculateMean, TranscriptomicsDataFilePath, TranscriptomicsColumnsToCalculateMean, RefreshGeneNames = TRUE)```
...
```UniProtKBAC2EnsemblID(UniProtKBAC.CSV, Wait = 5)```

```UniProtKBAC2EnsemblID``` utilizes the UniProt REST API to convert UniProtKB accession IDs in ```UniProtKBAC.CSV``` into the Ensembl IDs of corresponding genes. Once a query job is submitted, the status will be inquired every ```Wait``` seconds until the job is finished. The downloaded output is then parsed into a matrix with two columns named ```uniprotsptrembl``` and ```ensembl_gene_id``` (consistent with BioMart). Each row corresponds to a mapping relationship. If a protein/peptide is mapped to more than one gene, multiple rows might share the same UniProtKB accession ID in column 1 but have different Ensembl IDs in column 2.
